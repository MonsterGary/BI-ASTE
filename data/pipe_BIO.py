...
class BartBPEABSAPipe(Pipe):
    ...
    def process(self, data_bundle: DataBundle) -> DataBundle:
        ...
        def prepare_target(ins):
            ...
            matrx_mask = torch.eye(max_sequence_len).long()
    ...
    def _load(self, path):
        ...
        for ins in data:
            ...
            for triple in Triples:
                ...
                aspect['index']=int(triple['uid'][-1])
                opinion['index']=int(triple['uid'][-1])
                aspect_span,aterm = get_spans(triple['target_tags'],words)
                opinion_span,oterm = get_spans(triple['opinion_tags'],words)
                assert len(aspect_span)==1
                aspect['from']=aspect_span[0][0]
                aspect['to']=aspect_span[0][1]
                aspect['term']=aterm
                aspect['polarity']=triple['sentiment'][:3].upper()
                opinion['from']=opinion_span[0][0]
                opinion['to']=opinion_span[0][1]
                opinion['term']=oterm
            ...
            deprel=ins['deprel']
            ins = Instance(raw_words=words, aspects=aspects, opinions=opinions, postag=postag,head=head,deprel=deprel)
            ds.append(ins)
