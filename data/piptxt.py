...
from peng.atomic_single_example import initcomet,cometinference
...
class BartBPEABSAPipe(Pipe):
    def __init__(self, tokenizer='facebook/bart-base', opinion_first='A', vocab=None, paths=None):
        ...
        data,categories = get_transformed_io(paths)
        ...
        self.mapping.update({item: item for item in categories})
...
class ABSALoader(Loader):
    ...
    def _load(self, path):
        ds = DataSet()
        data, categories = get_transformed_io(path)
        train_data,_ =get_transformed_io(r'E:\PycharmWorkingSpace\BARTABSA-Syntactic\BARTABSA-main\data\acos\Restaurant-ACOS\rest16_quad_train.tsv')
        tokenizer = AutoTokenizer.from_pretrained('facebook/bart-base')
        model = BartModel.from_pretrained('huggingface/model', cache_dir='./huggingface/model', force_download=True,
                                          resume_download=True)
        data_sentence_vector = get_data_sentence_vector(train_data,tokenizer,model)
        for ins in data:
            sim_example = get_sim_example(data_sentence_vector,ins['raw_words'],tokenizer,model)[1]
            example= {'sentence': -1, 'target': -1}
            example['sentence']=train_data[sim_example]['raw_words']
            example['target']=train_data[sim_example]['term']
            ...
            # deprel=ins['deprel']
            ins = Instance(raw_words=raw_words,words=words, aspects=aspects, opinions=opinions,example=example)
            ds.append(ins)
...
def get_transformed_io(data_path):
    ...
    targets, categories = get_extraction_acos_targets(sents, labels)
    data=[]
...
def get_extraction_acos_targets(sents, labels):
    ...
    for i, label in enumerate(labels):
        ...
        for tri in label:
            ...
            s = '"'+tri[3]+'"'
            c = '"'+tri[2]+'"'
            if tri[2] not in categories:
                categories.append(tri[2])
            all_tri.append((a, b, c, s))
...
@cache_results('caches/example.pt', _refresh=False)
def get_data_sentence_vector(data,tokenizer,model):
    outputs=[]
    for datai in data:
        input = tokenizer(datai['raw_words'],return_tensors='pt')
        output = model(**input)[1].mean(dim=1).squeeze(dim=0)
        outputs.append(output)
    outputs=torch.stack(outputs)
    return outputs

def get_sim_example(data,sentence,tokenizer,model):
    input = tokenizer(sentence, return_tensors='pt')
    output = model(**input)[1].mean(dim=1).squeeze(dim=0)
    sim=F.cosine_similarity(data,output)
    values, indices = torch.topk(sim, 3)
    return indices
