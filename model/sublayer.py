...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(
        ...
        if structure_k is not None:  # [batch_size, seq_len, seq_len, dim]
            ...
            scores = (1-self.alpha)*scores + self.alpha * scores_k
            # scores = self.lamb1 * scores + self.lamb2 * scores_k

        if key_padding_mask is not None:  # padding mask
            key_padding_mask = key_padding_mask.unsqueeze(1)  # `[B, 1, 1, seq_len]`
        ...
        output = nn.functional.elu(context)
