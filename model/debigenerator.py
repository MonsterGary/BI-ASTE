...
class SequenceGeneratorModel(nn.Module):
    ...
    def __init__(self, seq2seq_model: Seq2SeqModel, bos_token_id, eos_token_id=None, max_length=30, max_len_a=0.0,
                 ...
                 restricter=None, opinion_first='A'):
        """
    ...
    def forward(self, src_tokens, tgt_tokens, src_seq_len=None, tgt_seq_len=None, first=None, head=None, pos_tag=None, senti_value=None,
                afsrc_tokens=None, aftgt_tokens=None, ofsrc_tokens=None, oftgt_tokens=None,word_index=None,
                afaspect_label=None,afopinion_label=None,ofaspect_label=None,ofopinion_label=None,pos_mask=None,head_len=None,
                word_pair_deprel=None, matrix_mask=None,deprel_ids=None,tree_based_word_pair_position=None,word_pair_position=None):
        """
        ...
        if 'A' in self.opinion_first:
            afpred, aloss = self.seq2seq_model(head,pos_tag,senti_value, afsrc_tokens, aftgt_tokens,
                                               src_seq_len, tgt_seq_len, first,word_index,afaspect_label,afopinion_label,
                                               head_len=head_len, word_pair_deprel=word_pair_deprel,
                                               matrix_mask=matrix_mask,
                                               deprel_ids=deprel_ids, tree_position=tree_based_word_pair_position,
                                               pair_position=word_pair_position
                                               )
        if 'O' in self.opinion_first:
            pass
            # ofpred, oloss = self.seq2seq_model(head,pos_tag,senti_value, ofsrc_tokens, oftgt_tokens,
            #                                    src_seq_len, tgt_seq_len, first,word_index,ofaspect_label,ofopinion_label,
            #                                    head_len=head_len, word_pair_deprel=word_pair_deprel,
            #                                    matrix_mask=matrix_mask,
            #                                    deprel_ids=deprel_ids, tree_position=tree_based_word_pair_position,
            #                                    pair_position=word_pair_position
            #                                    )
        # return afpred, ofpred, (aloss+oloss)/2
        return afpred, ofpred, aloss

    def predict(self,head,pos_tag, src_tokens, tgt_tokens, src_seq_len=None, first=None, senti_value=None,
                afsrc_tokens=None, aftgt_tokens=None, ofsrc_tokens=None, oftgt_tokens=None,word_index=None,pos_mask=None,head_len=None,
                word_pair_deprel=None, matrix_mask=None, deprel_ids=None,tree_based_word_pair_position=None,word_pair_position=None):
        """
        ...
        :return:
        """
        if 'A' == self.opinion_first:
            state, biaffine_edge_relu = self.seq2seq_model.prepare_state(head,pos_tag,senti_value,afsrc_tokens, src_seq_len, first,
                                                                         word_index=word_index, head_len=head_len,
                                                                         word_pair_deprel=word_pair_deprel,
                                                                         matrix_mask=matrix_mask, deprel_ids=deprel_ids,
                                                                         tree_position=tree_based_word_pair_position,
                                                                         pair_position=word_pair_position
                                                                         )
            result = self.generator.generate(state)  # the prompt is provided to the model
        if 'O' == self.opinion_first:
            state, biaffine_edge_relu = self.seq2seq_model.prepare_state(head, pos_tag,senti_value, ofsrc_tokens, src_seq_len,first,
                                                                         word_index=word_index, head_len=head_len,
                                                                         word_pair_deprel=word_pair_deprel,
                                                                         matrix_mask=matrix_mask, deprel_ids=deprel_ids,
                                                                         tree_position=tree_based_word_pair_position,
                                                                         pair_position=word_pair_position)
            result = self.generator.generate(state)  # the prompt is provided to the model
        if 'AO' == self.opinion_first:
            # aosrc_tokens=torch.cat((afsrc_tokens,ofsrc_tokens),dim=0)
            aosrc_tokens=afsrc_tokens
            # head=torch.cat((head,head),dim=0)
            # pos_tag=torch.cat((pos_tag,pos_tag),dim=0)
            # src_seq_len=torch.cat((src_seq_len,src_seq_len),dim=0)
            # senti_value=torch.cat((senti_value,senti_value),dim=0)
            #
            # word_index=torch.cat((word_index,word_index),dim=0)
            # head_len=torch.cat((head_len,head_len),dim=0)
            # word_pair_deprel=torch.cat((word_pair_deprel,word_pair_deprel),dim=0)
            # matrix_mask=torch.cat((matrix_mask,matrix_mask),dim=0)
            # deprel_ids=torch.cat((deprel_ids,deprel_ids),dim=0)
            # tree_based_word_pair_position=torch.cat((tree_based_word_pair_position,tree_based_word_pair_position),dim=0)
            # word_pair_position=torch.cat((word_pair_position,word_pair_position),dim=0)
            state, biaffine_edge_relu = self.seq2seq_model.prepare_state(head, pos_tag,senti_value, aosrc_tokens, src_seq_len,first,
                                                                         word_index=word_index, head_len=head_len,
                                                                         word_pair_deprel=word_pair_deprel,
                                                                         matrix_mask=matrix_mask, deprel_ids=deprel_ids,
                                                                         tree_position=tree_based_word_pair_position,
