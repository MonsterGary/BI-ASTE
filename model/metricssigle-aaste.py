...
class Seq2SeqSpanMetric(MetricBase):
    ...
    def evaluate(self, target_span, pred, tgt_tokens):
        ...
        pred_span,invalid = aste_parse(pred,pred_seq_len,self.word_start_index,target_span)
        ...
        for i, (ts, ps) in enumerate(zip(target_span, pred_span)):
            ...
            if self.opinin_first=='O':    #t=o o a a s
...
def aste_parse(pred, pred_seq_len, word_start_index,target):
    pred_span=[]
    invalid = 0
    pred_nums = len(pred)
    for i, ps in enumerate(pred.tolist()):
        ...
        if len(ps):
            for index, j in enumerate(ps):
                if j < word_start_index and j not in (5, 6, 7, 8):
                    cur_pair.append(j)
                    if len(cur_pair) != 5 or cur_pair[0] > cur_pair[1] or cur_pair[2] > cur_pair[3]:
                        invalid += 1
                    else:
