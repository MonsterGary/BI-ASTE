...
class Trainer(object):
        ...
        if isinstance(model, nn.parallel.DistributedDataParallel):  # 如果是分布式的
            ...
            if isinstance(sampler, ConstTokenNumSampler):  # 直接使用固定token数量的Sampler
                ...
                train_data = DataSetIter(train_data,
                                         ...
                                         pin_memory=self.pin_memory, drop_last=drop_last, timeout=0,
                                         worker_init_fn=None,
        ...
        with inner_tqdm(total=self.n_steps, postfix='loss:{0:<6.5f}', leave=False, dynamic_ncols=True,
            ...
            for epoch in range(self.epoch, self.n_epochs + 1):
                ...
                for batch_x, batch_y in self.data_iterator:
                    ...
                    predictionaf,predictionof,marker_loss = self._data_forward(self.model, batch_x)

                    # edit prediction
                    batch_yaf=batch_y.copy()
                    # batch_yof=batch_y.copy()
                    batch_yaf['tgt_tokens']=batch_y['aftgt_tokens']
                    # batch_yof['tgt_tokens']=batch_y['oftgt_tokens']
                    self.callback_manager.on_loss_begin(batch_yaf, predictionaf)
                    with self.auto_cast():
                        afloss = self._compute_loss(predictionaf, batch_yaf).mean()
                        # ofloss = self._compute_loss(predictionof, batch_yof).mean()
                        alpha=0.5
                        # loss=alpha*afloss+(1-alpha)*ofloss+marker_loss
                        loss=afloss+marker_loss
    ...
    def _data_forward(self, network, x):
        ...
        with self.auto_cast():
            yaf,yof,marker_loss = network(**x)
        if not isinstance(yaf, dict):
            raise TypeError(
                f"The return value of {_get_func_signature(self._forward_func)} should be dict, got {type(yaf)}.")
        return yaf,yof,marker_loss
...
def _check_code(dataset, model, losser, metrics, forward_func, batch_size=DEFAULT_CHECK_BATCH_SIZE,
    ...
    for batch_count, (batch_x, batch_y) in enumerate(_iter):
        ...
        afpred_dict,ofpred_dict,marker_loss = model(**refined_batch_x)
        func_signature = _get_func_signature(forward_func)
        if not isinstance(afpred_dict, dict):
            raise TypeError(f"The return value of {func_signature} should be `dict`, not `{type(afpred_dict)}`.")

        # loss check
        try:
            aftgt_y=batch_y.copy()
            oftgt_y=batch_y.copy()
            aftgt_y['tgt_tokens']=batch_y['aftgt_tokens']
            oftgt_y['tgt_tokens']=batch_y['oftgt_tokens']
            afloss = losser(afpred_dict, aftgt_y)
            # ofloss = losser(ofpred_dict, oftgt_y)
            alpha=0.5
            loss = afloss+marker_loss
            # check loss output
            if batch_count == 0:
